...
if [ -f "$STATE_DIR/state.json" ]; then
  SAVED_ROUND=$(grep '"round":' "$STATE_DIR/state.json" 2>/dev/null | head -1 | sed 's/.*"round": *//;s/[^0-9]//g')
...
只输出 N 行 REVIEW_RESULT，完成后输出 DONE。"
  ...
  for i in $(seq 1 "$N"); do
    ...
    if echo "$LINE" | grep -q "APPROVED"; then
      ...
      REASON=$(echo "$LINE" | sed 's/REVIEW_RESULT t'${i}': *REJECTED: *//')
  ...
  # === Phase 3 — 仅合并 APPROVED 且非 STALLED ===
  MERGED_COUNT=0
  cd "$WORKDIR"
  for i in $(seq 1 "$N"); do
    WT_DIR="${WORKTREES[$((i-1))]}"
    ...
    if [ "$DECISION" = "APPROVED" ] && [ "$STALLED" != "1" ]; then
  ...
  RECORD="{\"round\":${ROUND},\"stage\":\"${STAGE}\",\"duration_sec\":${DURATION},\"tasks\":${N},\"merged\":${MERGED_COUNT},\"diff_stat\":\"${DIFF_STAT}\",\"timestamp\":\"$(date -u +"%Y-%m-%dT%H:%M:%SZ")\"}"
  if [ -f "$STATE_DIR/ledger.json" ]; then
    sed -i "s/\]$/,${RECORD}]/" "$STATE_DIR/ledger.json" 2>/dev/null
  else
  ...
  if [ "$RECENT_FAILS" -eq "$N" ]; then
    echo "[$(date)] MAIN_LOOP: 本轮全部 429，暂停 30 分钟" >> "$LOG"
    sleep 1800
  fi
