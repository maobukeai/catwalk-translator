...
### 2. ⚡ 快慢双流渐进呈现（在线 0.15s 秒出 + AI 慢通道无缝升级）
- **在线引擎并发大竞速**：首屏以毫秒级速度并发请求 Google、Bing、有道等多引擎，谁最快率先上屏，零等待感。
- **大模型后台异步精翻**：后台静默进行大模型高阶推导，完成后平滑无缝升级主译文并点亮 `✨` 专属徽章。
- **生词本容量扩容至 2,000 条**：支持优质生词智能甄选自动收藏（剔除无价值日常词，锁定 3D/CG 术语与 AI 精翻），星标词终身防淘汰。
...
### 🚀 极速一键启动 (推荐)
- **Windows 用户**：直接双击根目录下的 [`启动猫步翻译.bat`](file:///c:/Users/20269/Desktop/项目文件夹/翻译软件/启动猫步翻译.bat) 即可自动检查环境、安装依赖并启动客户端；
- **PowerShell / 终端用户**：运行 [`启动猫步翻译.ps1`](file:///c:/Users/20269/Desktop/项目文件夹/翻译软件/启动猫步翻译.ps1) 即可进入带菜单的交互式管理工具（支持一键调试、全量测试与安装包打包）。
